...
def draw_fork(x, y, scale=1.0, angle=0):
    fork = Image.new("RGBA", (60, 160), (0, 0, 0, 0))
    fd = ImageDraw.Draw(fork)
    fd.rounded_rectangle([25, 60, 35, 155], radius=4, fill=color_main)
    for tx in [12, 22, 32, 42]:
        fd.rounded_rectangle([tx, 5, tx + 6, 65], radius=3, fill=color_main)
    fd.rounded_rectangle([10, 55, 50, 65], radius=3, fill=color_main)
    fork = fork.resize((int(60 * scale), int(160 * scale)), Image.LANCZOS)
    if angle:
        fork = fork.rotate(angle, expand=True, resample=Image.BICUBIC)
    final_img.paste(fork, (x - fork.width // 2, y - fork.height // 2), fork)

def draw_knife(x, y, scale=1.0, angle=0):
    knife = Image.new("RGBA", (50, 160), (0, 0, 0, 0))
    kd = ImageDraw.Draw(knife)
    kd.rounded_rectangle([18, 80, 32, 155], radius=5, fill=color_main)
    kd.polygon([(25, 5), (38, 20), (35, 78), (18, 78)], fill=color_main)
    kd.line([(28, 15), (30, 70)], fill=(255, 255, 255, 100), width=2)
    knife = knife.resize((int(50 * scale), int(160 * scale)), Image.LANCZOS)
    if angle:
        knife = knife.rotate(angle, expand=True, resample=Image.BICUBIC)
    final_img.paste(knife, (x - knife.width // 2, y - knife.height // 2), knife)

def draw_spoon(x, y, scale=1.0, angle=0):
    spoon = Image.new("RGBA", (50, 160), (0, 0, 0, 0))
    sd = ImageDraw.Draw(spoon)
    sd.rounded_rectangle([20, 65, 30, 155], radius=4, fill=color_main)
    sd.ellipse([8, 5, 42, 70], fill=color_main)
    sd.ellipse([14, 12, 36, 55], fill=(220, 180, 160))
    spoon = spoon.resize((int(50 * scale), int(160 * scale)), Image.LANCZOS)
    if angle:
        spoon = spoon.rotate(angle, expand=True, resample=Image.BICUBIC)
    final_img.paste(spoon, (x - spoon.width // 2, y - spoon.height // 2), spoon)
...
# === Сохраняем ===
output = "J:/Cloude/3D AXROR/qr-code-afra.png"
