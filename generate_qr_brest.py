...
qr.add_data('https://armenu.shumtuber.uz/brest.html')
qr.make(fit=True)

# Цвета Brest — тёмно-зелёный + золото
color_main = (42, 60, 26)        # тёмный оливковый
color_dark = (26, 38, 18)        # глубокий лесной зелёный
color_accent = (200, 160, 80)    # тёплое золото
...
# === 3. Логотип Brest в центре ===
logo_src = Image.open("J:/Cloude/3D AXROR/brest.png").convert("RGBA")
logo_size = int(qr_w * 0.30)
...
bg_draw.rounded_rectangle([0, 0, mask_w, mask_h], radius=16, fill=(248, 252, 242, 255))
...
final_img = Image.new("RGBA", (final_w, final_h), (248, 252, 242))
df = ImageDraw.Draw(final_img)
df.rounded_rectangle([12, 12, final_w - 12, final_h - 12], radius=30, outline=color_main, width=4)
df.rounded_rectangle([22, 22, final_w - 22, final_h - 22], radius=25, outline=(42, 60, 26, 80), width=1)
...
stw = bbox2[2] - bbox2[0]
df.text(((final_w - stw) // 2, ty + 65), sub, fill=(90, 110, 80), font=font_sub)

output = "J:/Cloude/3D AXROR/qr-code-brest.png"
